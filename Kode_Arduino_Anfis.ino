...
void loop() {
  ...
  if (Serial.available()) {
    ...
    // Kendali motor berdasarkan perintah menggunakan switch-case
    switch (command) {
      case 'F':  // Forward
        moveForward();
        break;
      ...
      case 'L':  // Left
        turnRight();  // Kembali maju
        delay(350);  // Belok selama 500ms
        stopMotors();
        delay(100);
        break;

      case 'R':  // Right
        turnLeft();  // Kembali maju
        delay(330);  // Belok selama 500ms
        stopMotors();
        delay(100);
        break;
...
void moveForward() {
  ...
  analogWrite(ENA, 245);  // Kecepatan motor A
  analogWrite(ENB, 245);  // Kecepatan motor B
}
